...
function makeLog(id, file, fh, maxBytes, mask) {
  ...
  // Output arrives in chunks; a secret can straddle two. Hold back a tail so the
  // join is scanned once the next chunk arrives.
  let held = '';
...
- Produces:
  ...
  - `runOnFailure(ctx, repo, { attemptId, outcome, sha, releaseId, logFile }) → Promise<void>`: runs `ON_FAILURE` if set, 60 s cap, writes one `notified` event; never throws for a failing notifier
...
export async function runEntry(ctx, entry) {
  ...
  // Step 0: open. Both env files are read now so their values can be masked in
  // everything this attempt writes, whichever phase prints them.
  const mask = [...(await loadEnvFile(repo.buildEnvFile).catch(() => new Map())).values(), ...(await loadEnvFile(repo.deployEnvFile).catch(() => new Map())).values()];
  ...
  if (outcome !== 'ok' && outcome !== 'skipped') {
    await runOnFailure(ctx, repo, { attemptId: log.id, outcome, sha: last.sha, releaseId: last.release, logFile: log.file });
  }
  return outcome;
}

export async function runOnFailure(ctx, repo, { attemptId, outcome, sha, releaseId, logFile }) {
  if (!repo.onFailure) return;
  const { paths: p } = ctx;
  const logDir = p.repoLog(repo.name);
  const { env } = await commandEnv(ctx, repo, 'deploy', { releaseId: releaseId ?? '', sha: sha ?? '', previousSha: '', attemptId }).catch(() => ({ env: { PATH: '/usr/local/bin:/usr/bin:/bin', HOME: p.lib, DEPLOY_NAME: repo.name } }));
  env.DEPLOY_OUTCOME = outcome;
  env.DEPLOY_LOG = logFile ?? '';
  let out = '';
  const r = await runCommand({ command: repo.onFailure, cwd: p.repoDir(repo.name), env, timeoutSec: 60, onOutput: (c) => { out += c; }, signal: ctx.signal, graceMs: ctx.graceMs });
  const code = r.timedOut ? 'timeout' : r.code ?? `signal ${r.signal}`;
  await appendEvent(logDir, 'notified', `${attemptId} exit ${code}${out.trim() ? `  ${out.trim().split('\n')[0].slice(0, 200)}` : ''}`);
}
...
Run: `node --test test/run.test.mjs`
Expected: 18 passing. The `shutdown mid-build` test takes about a second; the `output past LOG_MAX_BYTES` test relies on `yes` and `head` being on PATH, which they are on Linux and macOS.
...
- Produces:
  - `runCheck(ctx, { setRemote }) → Promise<{ rows: [label, text][], passed: boolean, behind: boolean }>` (`behind` when the branch head differs from the live sha, or `pending` is set) where `ctx = { paths, repo, journal }`. What `deployd check` does, run on the worker so it reads the deploy key as `deployd` and never races a fetch.
...
export async function serve({ paths: p, journal = (l) => process.stderr.write(`${l}\n`) }) {
  ...
  const hook = createHookServer({
    ...
    onPush: async (repo, info) => {
      ...
      const state = await readState(p.repoDir(repo.name));
      if (state.github_id === null && info.id !== null) {
        state.github_id = info.id;
        await writeState(p.repoDir(repo.name), state);
      }
      if (state.pending) {
