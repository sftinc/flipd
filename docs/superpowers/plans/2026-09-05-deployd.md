...
- Produces:
  ...
  - `git(args, { env, onOutput, signal }) → Promise<{ code, stdout, stderr, aborted }>`; an aborted signal kills the child
...
export function git(args, { env, onOutput, signal } = {}) {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code, sig) => {
      signal?.removeEventListener('abort', onAbort);
      resolve({ code, stdout, stderr, aborted: Boolean(signal?.aborted && sig) });
    });
  });
}
...
**Interfaces:**
- Produces: `createQueue(runner, { onError }) → { enqueue(entry) → { queued: boolean, reason? }, enqueueIfIdle(entry) → { queued, reason? }, running() → entry | null, queued() → entry[], isRunning(name), isQueued(name), protectedTargets(name) → string[], drain() → Promise<void>, stop() }`. `entry = { kind: 'webhook' | 'manual' | 'rollback' | 'check', name, target?, ... }`. `runner(entry) → Promise<void>` is awaited one at a time. `enqueueIfIdle` queues only when nothing is running and nothing is queued, and otherwise reports `busy: running <name>, N queued`. After `stop()`, `enqueue` refuses, nothing is re-queued, and `drain()` resolves once the running entry ends.
...
- Produces:
  ...
  - `commandEnv(ctx, repo, phase, { releaseId, sha, previousSha, attemptId }) → Promise<{ env, keys, refused, file }>`; deployd's own variables win and `refused` lists the file keys that tried to replace one
...
export async function commandEnv(ctx, repo, phase, { releaseId, sha, previousSha, attemptId }) {
  ...
  // deployd's own variables win. An env file that tries to replace PATH or a
  // DEPLOY_* name gets a warning line, not a different environment.
  const refused = [];
  for (const [k, v] of extra) {
    if (k in env) refused.push(k);
    else env[k] = v;
  }
...
export async function runEntry(ctx, entry) {
  ...
  const runPhase = async (which, command, cwd, ids) => {
    phase = which;
    const { env, keys, refused, file } = await commandEnv(ctx, repo, which, ids);
    ...
    if (entry.kind === 'rollback') {
      ...
      try {
        if (!(await isBareRepo(gitDir))) {
          await fs.rm(gitDir, { recursive: true, force: true });   // a half-clone from a crash, or nothing
          await log.line(`clone --bare ${repo.repo}`);
          await cloneBare(repo.repo, gitDir, gitRun);
        } else {
...
Run: `node --test test/run.test.mjs`
Expected: 16 passing. The `shutdown mid-build` test takes about a second; the `output past LOG_MAX_BYTES` test relies on `yes` and `head` being on PATH, which they are on Linux and macOS.
...
test('bodies over 1 MiB get 413 before signature checking, declared or chunked', async () => {
  ...
  try {
    ...
    const stream = new ReadableStream({ start(c) { c.enqueue(big.subarray(0, 600000)); c.enqueue(big.subarray(600000)); c.close(); } });
...
test('a throwing onPush is a 500, and the server survives it', async () => {
  const lines = [];
  const h = await listen({ secret: 's', loadRepos: async () => ({ repos, errors: [] }), onPush: async () => { throw new Error('kaboom'); }, journal: (l) => lines.push(l) });
  try {
...
function readBody(req) {
  return new Promise((resolve, reject) => {
    ...
    req.on('data', (c) => {
      size += c.length;
      if (size > MAX_BODY) {
        // Stop reading, but leave the socket alive so the 413 can be written.
        req.pause();
...
export function createHookServer({ secret, loadRepos, onPush, journal }) {
  return http.createServer((req, res) => {
    ...
    handle(req, res, { secret, loadRepos, onPush, journal }).catch((e) => {
...
async function handle(req, res, { secret, loadRepos, onPush, journal }) {
...
- Produces:
  - `runCheck(ctx, { setRemote }) → Promise<{ rows: [label, text][], passed: boolean }>` where `ctx = { paths, repo, journal }`. What `deployd check` does, run on the worker so it reads the deploy key as `deployd` and never races a fetch.
...
test('reconcile: a real run that was interrupted mid-deploy reads as interrupted', async () => {
  ...
  await writeMain(p);
  const ac = new AbortController();
  const ctx = { paths: p, main: await loadMain(p), repo: await loadRepo(p, 'r'), now: () => new Date(), protectedTargets: () => [], signal: ac.signal, journal: () => {}, graceMs: 200 };
...
export async function runCheck({ paths: p, repo }, { setRemote = false } = {}) {
  const rows = [];
  let passed = true;
...
export async function serve({ paths: p, journal = (l) => process.stderr.write(`${l}\n`) }) {
  ...
  const queue = createQueue(async (entry) => {
    ...
    if (entry.kind === 'check') {
      const result = await runCheck({ paths: p, repo, journal }, { setRemote: entry.setRemote });
...
- Produces:
  ...
  - `deployd check <name> [--set-remote]` sends `{cmd:'check'}` over the socket with a 10-minute timeout, prints the rows, exit 0 when `passed`, 1 otherwise, 3 when the service is down, and 1 with the `busy:` line when the worker is not idle.
...
test('check: prints the rows the service returns, exits by passed, reports busy and service down', async () => {
  ...
  const o = io();
  assert.equal(await check(['r', '--set-remote'], { paths: p, ...o, sendOverride: send({ ok: true, passed: true, rows: [['config', 'ok'], ['main', '5ac3c5a  (live: none)  behind']] }) }), 0);
