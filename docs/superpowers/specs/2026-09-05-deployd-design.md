...
| Release directory name | `<run-id>`, which is the timestamp plus short sha | The sha alone (a forced run at the live sha would replace the directory `current` points at) |
...
- No polling fallback. GitHub delivers a webhook once and does not retry on its own,
  ...
  lost. Recovery is `deployd run <name>`, and `deployd status` is the habit that
  catches it.
...
/var/lib/deployd/<name>/
  ...
  releases/<run-id>/              # a fresh worktree per run; the build happens here
  current -> releases/<run-id>    # flipped only after the build passes
...
/var/log/deployd/<name>/
  ...
  <run-id>.log                    # one file per run, complete build and deploy output
...
`<run-id>` is `<UTC time>-<short sha>`, for example `2026-09-05T08-14-02Z-5ac3c5a`.
It names the release directory and the run log, so the two are always findable from
each other, and it is unique per run, so a second build of the same sha never
replaces a directory that `current` or `previous` points at. The full 40-character
sha lives in `state.json` beside each run id.
...
| `BUILD` | yes | | Run through `sh -c` in `releases/<run-id>/<ROOT>` |
...
- A **webhook** entry is dropped if any entry for that name is already queued. If
  that repo is currently running, a "run again after" flag is set on it instead;
  when the run finishes, one webhook entry is queued. Three pushes during a build
  produce one follow-up run.
...
- A **rollback** entry is always appended. Its target release is resolved when the
  ...
  change what the rollback flips to. If there is nothing to roll back to, the command
...
has a truthful target, and so that a crash mid-run is visible at startup.

```json
{
  "live":     "<run-id whose DEPLOY last succeeded, or null>",
  "previous": "<the live before that, or null>",
  "pending":  "<run-id flipped to but not yet confirmed, or null>",
  "releases": {
    "<run-id>": { "sha": "<40 hex>", "root": "<ROOT>", "deploy": "<DEPLOY>",
                  "built": "<ISO-time>" }
  },
  "last": {
    "id": "<run-id>", "sha": "<40 hex>", "trigger": "webhook | manual | rollback",
    "outcome": "ok | build failed | deploy failed | fetch failed | checkout failed | skipped | interrupted",
    "started": "<ISO-time>", "finished": "<ISO-time>", "log": "<absolute path or null>"
  }
}
```
...
1. **Fetch.** If `git/` does not exist, `git clone --bare` creates it. Then
...
3. **Checkout.** A run id is minted. `git worktree add --detach releases/<run-id>
   <sha>` from the bare clone. A worktree rather than an archive so that a build
...
4. **Build.** `sh -c "$BUILD"` in `releases/<run-id>/<ROOT>`, stdout and stderr
...
5. **Flip.** `state.pending` is set to `<run-id>` and written to disk *first*. Then
   a temporary symlink `current.tmp -> releases/<run-id>` is created and renamed
   over `current`, so there is no instant at which `current` is absent. `live` and
   `previous` do not move yet.

6. **Deploy.** `sh -c "$DEPLOY"` in the same directory, same streaming, same
   timeout. *Success:* `previous` takes the old `live`, `live` takes `<run-id>`,
...
7. **Prune.** Release directories are listed newest-first. `live`, `previous` and
   `pending` are never candidates. Of the rest, the newest `KEEP` are kept and the
...
8. **Close.** The run log gets its summary line; `events.log` gets `finished
   <run-id> <outcome> <duration> <logfile>`; `state.last` is written.
...
`previous`. It queues a run whose steps are only 5 and 6, using the target's stored
`root` and `deploy`. On success, if the target was `previous`, `live` and `previous`
swap; if the target was `live`, `pending` is simply cleared. Nothing is fetched or
built. If the target is null, or its directory has gone, the command refuses and
says so.
...
that repo's `events.log` as `interrupted <run-id>`, `state.last.outcome` is set to
`interrupted` if it was not already final, and the repo is treated exactly as after
a `deploy failed`: `current` is left where it is, and webhook runs are refused until
a rollback or a manual run settles it.
...
| `DEPLOY_RELEASE_DIR` | absolute path of `releases/<run-id>` |
...
**One file per run.** `/var/log/deployd/<name>/<run-id>.log`, for example
`2026-09-05T08-14-02Z-5ac3c5a.log`. Plain text, append-only, written as the run happens so a `tail -f` shows a build in
...
`skipped`, `fetch-failed`, `refused`, `interrupted`, `rollback`. Every line about a run names its log file.
...
| `deployd check <name>` | no | Parses the config, confirms the key exists, creates the bare clone if missing, runs `git ls-remote` with the key and prints the branch head sha. No fetch into the shared clone, so it cannot race a run in progress. No build |
...
| `deployd status [name]` | no | One row per repo: branch, live sha, last outcome and time, queued or running. `PENDING <run-id>` in capitals when a flip is unconfirmed |
...
`/etc/deployd` is root-owned, so `add` and `remove` are run with sudo. `add` creates
...
3. Creates the three trees: `/etc/deployd` and `repos/` owned `root:deployd` mode
   `0750`; `/var/lib/deployd` and `/var/log/deployd` owned `deployd:deployd`.
...
- **Integration.** A temp directory holding a bare repo with two commits, a config
  ...
  `previous`); a prune past `KEEP` after a run of failures; a `pending` left in
  state at startup; output past `LOG_MAX_BYTES`.
