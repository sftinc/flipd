...
| `PUBLIC_HOST` | none | The hostname GitHub calls; written by `install.sh --host`, used only to print the webhook URL |
...
Re-running it is safe: every step is skip-if-present, and `--host` with the same
