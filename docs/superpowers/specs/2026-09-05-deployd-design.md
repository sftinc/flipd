...
| Secrets for commands | Two env files per repo, build and deploy, found by name | One shared file (every `npm ci` postinstall could read the deploy secret) |
...
They are split because `npm ci` runs every dependency's install scripts, and a
deploy credential in BUILD's environment is a deploy credential every package on
the box can read. With the split, a private-registry token goes in `.build` and a
`wrangler` token goes in `.deploy`, and neither command sees the other's.
...
```
deploy.example.com {
    reverse_proxy /deploy 127.0.0.1:9000
    respond 404
}
```
...
by deployd; `install.sh` drops a logrotate rule that rotates it monthly and keeps
twelve.
