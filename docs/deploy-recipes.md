...
The recipes above put the release where something can serve it. This one is the
other half: the Caddy site block that makes it reachable from outside. It is
the only section here that is not a `DEPLOY` command — nothing in it runs at
deploy time — but `install.sh --host` already put Caddy on the box for the
webhook, and the same Caddy can serve the app.
