...
| Key | Default | Meaning |
|---|---|---|
| `REPO` | required | The URL to fetch. A push is matched by comparing this value to the payload's `ssh_url` as host/owner/repo, so case, a `.git` suffix and the `git@host:` versus `ssh://git@host:port/` forms do not matter. **Every** repo file naming this repository and branch is matched, not the first — that is how a monorepo deploys more than one project ([Several projects in one repository](adding-a-repo.md#several-projects-in-one-repository)); a URL that is not of that shape (`file://`) is compared as a lowercased string. A repository renamed on its forge is matched by its numeric id once one webhook run has recorded it, scoped to the same host when both hosts are known, and the attempt log says to update `REPO`. `add` rewrites a GitHub `https://` URL to `git@github.com:owner/repo.git` (fetch needs the SSH form) and, with an account, writes the URL the forge itself renders. A URL carrying `user:password@` or `token@` is refused; use the deploy key. |
| `BRANCH` | `main` | The branch to follow. A push to any other branch is answered `ignored`. Pointing this at a branch you push to deliberately — `BRANCH=deploy`, then `git push origin main:deploy` — is the simplest way to keep everyday backup pushes from deploying: [operating.md](operating.md#pushing-without-deploying). |
