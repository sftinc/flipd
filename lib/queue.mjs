...
export function createQueue(runner, { onError = () => {} } = {}) {
  ...
  async function kick() {
    ...
    // The Set kept only the name, never the entry, so the rerun cannot say
    // who asked for it; `coalesced` says what the queue knows — a trigger
    // arrived mid-build and this is the catch-up.
    if (runAgain.delete(current.name) && !stopped) enqueue({ kind: 'webhook', name: current.name, via: 'coalesced' });
    kick();
  }
