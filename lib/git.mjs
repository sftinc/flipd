...
export function gitEnv({ key, knownHosts, home }) {
  return {
    ...
    // No /etc/gitconfig: the rest of this environment is built from scratch,
    // and an ambient system setting (a protocol.file.allow, a credential
    // helper) reaching a fetch is the same class of surprise. Per-repo git
    // settings for flipd go in $HOME/.gitconfig, which is still read.
...
// Not redundant with `--init` below: `submodule.<name>.url` lives in the shared
// bare clone's config (git/config in a linked worktree, not anything per-worktree),
// and `submodule init` deliberately does not overwrite an entry that already
// exists there. Without this, a repo that repoints a submodule keeps fetching
// the URL an earlier release recorded, forever. `sync` rewrites that entry from
// the worktree's own .gitmodules every time, so each release's checkout reads
// the URL that release actually names.
export const submoduleSync = (worktree, opts) => gitOk(['-C', worktree, 'submodule', 'sync', '--recursive', '--quiet'], opts);
