...
export function parseKV(text, allowedKeys) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // The line's own text is never echoed. Every caller of this parser reads a
    // file that can hold a secret — remote-deploy.conf holds WEBHOOK_SECRET, an
    // env file holds nothing but values — and a line that failed to parse is
    // exactly the line no mask can cover, since a file that did not parse
    // contributes nothing to the attempt log's mask. The line number is enough
    // for an operator with the file in front of them.
