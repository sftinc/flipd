  ...
  const hook = createHookServer({
    ...
    onPush: async (repo, info) => {
      ...
      try {
        ...
        journal(`[${repo.name}] refused a push: ${e.message}; repair or move that file aside, then remote-deploy run ${repo.name}`);
        await appendEvent(p.repoLog(repo.name), 'refused', `state.json is unreadable; repair it, then run remote-deploy run ${repo.name}`)
