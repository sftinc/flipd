...
export async function serve({ paths: p, journal = (l) => process.stderr.write(`${l}\n`) }) {
  ...
  const hook = createHookServer({
    ...
    onPush: async (repo, info) => {
      // Both fields are payload text. appendEvent writes raw by contract (see
      // lib/log.mjs), so the caller cleans: 39 is GitHub's maximum login
      // length, and a sha that is not one is worth seeing rather than hiding.
      await appendEvent(p.repoLog(repo.name), 'webhook', `${cleanForLog(info.sha)} ${cleanForLog(info.pusher, 39)}`.trim());
      let state;
      try {
