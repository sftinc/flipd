...
export function findRepoFor(p, journal) {
  return async ({ sshUrl, branch, id }) => {
    ...
    // Case-insensitive: GitHub renders ssh_url in the repository's canonical
    // case, an operator types whatever they type, and hostnames and GitHub
    // and GitLab paths are all case-insensitive. Without this a lowercase
    // REPO passes every check row (ls-remote is case-insensitive too) and
    // never matches a push, and github_id is never recorded to fall back on.
    const wanted = sshUrl.toLowerCase();
    const direct = repos.find((r) => r.repo.toLowerCase() === wanted && r.branch === branch);
