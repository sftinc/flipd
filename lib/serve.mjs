...
export async function serve({ paths: p, journal = (l) => process.stderr.write(`${l}\n`) }) {
  ...
  // latency to nothing systemd is timing.
  for (const job of pendingNotifications) {
    loadRepo(p, job.name)
      .then((repo) => runOnFailure({ paths: p, journal }, repo, job))
  ...
  const close = () => {
    ...
    closing = (async () => {
      ...
      abort.abort();
      // Drain the in-flight deploy *before* touching either server. Closing
      // the socket first would wait on it (net.Server#close waits for every
      // open connection to end), and a stuck or hostile client could then
      // starve this drain indefinitely — systemd hits its stop timeout and
      // SIGKILLs the process out from under the very deploy the drain exists
      // to protect. The hook server is not exposed to this (checked
      // empirically), but ordering both the same way costs nothing.
      await queue.drain();
      await new Promise((r) => hook.close(r));
      await new Promise((resolve) => {
        ...
        // open for more than an instant. Force any that remain — a wedged or
        // hostile client — closed, so shutdown itself stays bounded too.
