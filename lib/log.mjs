...
function makeLog(id, file, fh, maxBytes, mask) {
  ...
  return {
    id,
    file,
    // The same scrubber the attempt log writes through, exposed so that the
    // other sink for this attempt's text — events.log, written by appendEvent,
    // which has no mask of its own — masks identically rather than approximately.
    scrub,
    line(text) {
      // flipd's own lines are never dropped, but no single line may carry an
...
// Writes raw. There is no mask to apply here: a mask belongs to one attempt, and
// this function is also called from the hook handler and from startup, where no
// attempt is open. Any caller whose `details` can carry command output or the
// text of a failed command scrubs it first, through that attempt's own scrubber
// (`log.scrub`, or runOnFailure's) — see lib/run.mjs.
