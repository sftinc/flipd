...
export function createHookServer({ secret, findRepos, onPush, journal }) {
  const server = http.createServer((req, res) => {
    ...
    handle(req, res, { secret, findRepos, onPush, journal }).catch((e) => {
...
async function handle(req, res, { secret, findRepos, onPush, journal }) {
  ...
  // several confs sharing one REPO, and one push is all of theirs.
  const repos = await findRepos({ sshUrl, branch, id });
  if (repos.length === 0) {
  ...
  const { status, body: text } = await onPush(repos, info);
