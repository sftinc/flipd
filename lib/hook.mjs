...
export function createHookServer({ secret, findRepo, onPush, journal }) {
  ...
  // is here. Unbounded connections make that unbounded memory; 16 bounds the
  // worst case at 16 * MAX_BODY. Behind Caddy the listener sees one delivery at
  // a time, so normal operation never approaches this. Over the cap Node
  // destroys the socket at once and the proxy answers 502, which is a lost
  // delivery -- the trade being that a bounded refusal under flood is better
