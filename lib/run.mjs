...
export async function runEntry(ctx, entry) {
      ...
      try {
        if (!(await isBareRepo(gitDir))) {
          ...
          if (origin !== repo.repo) {
            ...
            throw new Stop('fetch failed', `REPO changed: clone has ${redactUserinfo(origin)}, config says ${repo.repo}; run remote-deploy check ${name} --set-remote`);
          }
        }
  ...
  try {
    ...
    // a failed command, and events.log is the log remote-deploy never prunes
...
async function failUnreadableState(ctx, entry, { dir, logDir, name, error }) {
  ...
  await appendEvent(logDir, 'started', `${log.id} ${entry.kind} ${log.file}`);
  await log.line(`remote-deploy attempt ${log.id}  name=${name}  trigger=${entry.kind}`);
  ...
  await log.line(`next: repair or move aside ${path.join(dir, 'state.json')}, then remote-deploy run ${name}`);
  await log.close();
