...
import { gitEnv, cloneBare, remoteUrl, fetchBranch, changedFiles, worktreeAdd, worktreeRemove, worktreePrune, submoduleSync, submoduleUpdate, shortSha, isBareRepo, redactUserinfo, GitError } from './git.mjs';
...
export function commandEnv(ctx, repo, phase, { releaseId, sha, previousSha, attemptId, currentReleaseId, currentReleaseDir }, extra = new Map()) {
  ...
  const env = {
    ...
    // STOP only: the release whose process is being stopped. The target
    // (DEPLOY_RELEASE_*) is where the attempt is going; this is where it is.
    // Both empty on a first deploy. Under the DEPLOY_ prefix so an env file
    // is refused from setting them, like every other flipd-owned name.
    ...(phase === 'stop' ? { DEPLOY_CURRENT_RELEASE_DIR: currentReleaseDir ?? '', DEPLOY_CURRENT_RELEASE_ID: currentReleaseId ?? '' } : {}),
  };
...
export async function runEntry(ctx, entry) {
  ...
  // What a reader sees as the trigger. `via` is a label and nothing else —
  // an SSH trigger ('ssh') or the queue's own catch-up rerun ('coalesced')
  // is a webhook in every rule above and below; only the record differs.
  const trigger = entry.via ?? entry.kind;
  const last = { attempt: log.id, trigger, sha: null, release: null, outcome: null, started: started.toISOString(), finished: null, log: log.file };
  ...
  await appendEvent(logDir, 'started', `${log.id} ${trigger} ${log.file}`);
  await log.line(`flipd attempt ${log.id}  name=${name}  trigger=${trigger}${entry.target ? `  target release=${entry.target}` : ''}`);
  ...
  const runPhase = async (which, command, cwd, ids) => {
    ...
    // `stop` is deploy-side: it reads the deploy env file, and a file that
    // fails to parse fails this phase under this phase's own name.
    const side = which === 'build' ? 'build' : 'deploy';
    const envError = envFiles[`${side}Error`];
    ...
    if (envError) throw new Stop(`${which} failed`, `env file: ${envError.message}`);
    const { env, keys, refused, file } = commandEnv(ctx, repo, which, ids, envFiles[side]);
  ...
  // STOP: the running process gets its chance to finish before anything is
  // switched. This lands before flip() on purpose — a process that will not
  // stop must leave `current`, live, previous and pending exactly as they
  // were, and must not create a pending. flipd never touches the served
  // process itself; on timeout it kills this command's own group, nothing
  // else, so a script cut off mid-drain is the script's problem to guard
  // against (docs/build-and-deploy.md says how).
  const stop = async (releaseId, rel, previousSha) => {
    if (!repo.stop) return;
    phase = 'stop';
    // Gated on the kind as well as the flag: a webhook entry is built with
    // fixed fields in serve.mjs and never carries `now`, and this keeps it that
    // way even for a caller that hands runEntry an entry by hand.
    if (entry.now === true && entry.kind !== 'webhook') { await log.line('stop skipped: --now'); return; }
    if (ctx.signal?.aborted) throw new Stop('interrupted', 'stop not started: service shutting down');
    // Where it runs: the release `current` points at, under that release's
    // recorded ROOT, so an in-repo drain script is the copy that matches the
    // process it is stopping. The symlink itself, not `pending ?? live`:
    // flip() writes `pending` before the rename, and serve.mjs's startup
    // reads the symlink for the same reason. No symlink means a first deploy,
    // and the target's own root is the only directory there is.
    let currentId = null;
    try { currentId = path.basename(await fs.readlink(path.join(dir, 'current'))); } catch { /* first deploy */ }
    let currentDir = '';
    let cwd;
    if (currentId) {
      const cur = state.releases[currentId];
      if (!cur) throw new Stop('stop failed', `current points at ${currentId}, which state.json does not list; nowhere to run STOP`);
      currentDir = path.join(dir, 'releases', currentId);
      cwd = path.join(currentDir, cur.root);
    } else {
      cwd = path.join(dir, 'releases', releaseId, rel.root);
    }
    await step('stop');
    await log.line(`stop in ${cwd}`);
    const ok = await runPhase('stop', repo.stop, cwd, { releaseId, sha: rel.sha, previousSha, attemptId: log.id, currentReleaseId: currentId ?? '', currentReleaseDir: currentDir });
    await done('stop');
    if (!ok) throw new Stop('stop failed', `STOP exited ${exitCodes.stop}`);
  };
  ...
  try {
    if (entry.kind === 'rollback') {
      ...
      await stop(entry.target, rel, liveSha);
      ...
      try {
        ...
        // Gated on the file, not on a config key: a repo with a submodule that
        // is not initialised builds from an empty directory and the failure
        // looks like a broken build. A repo without one pays one stat and
        // logs nothing. Storage is per-worktree (git/worktrees/<id>/modules),
        // so every release clones afresh and prune removes it with the release.
        if (await fs.stat(path.join(relDir, '.gitmodules')).then(() => true, () => false)) {
          await log.line('submodules: .gitmodules present');
          // .gitmodules is repo content, not REPO: anyone who can push can write a
          // URL with userinfo in it, and git echoes that URL verbatim on a clone
          // failure. redactUserinfo per chunk is the same best-effort masking as
          // the log's own carry-buffer limitation (lib/log.mjs) — a credential
          // split across a chunk boundary can still slip through — but it is what
          // every other sink in this file already relies on for a URL that reaches
          // git some other way than REPO.
          const submoduleRun = { ...gitRun, onOutput: (c) => log.output(redactUserinfo(c)) };
          await submoduleSync(relDir, submoduleRun);
          await submoduleUpdate(relDir, submoduleRun);
        }
      } catch (e) {
        if (e instanceof GitError && e.aborted) throw new Stop('interrupted', 'checkout interrupted by service shutdown');
        if (e instanceof GitError) throw new Stop('checkout failed', redactUserinfo(`${e.message}\n${e.stderr}`));
        throw e;
      }
      ...
      // Steps 5, 6 and 7.
      await stop(releaseId, state.releases[releaseId], liveSha);
    ...
    if (e instanceof Stop) {
      ...
      outcome = { fetch: 'fetch failed', checkout: 'checkout failed', build: 'build failed', stop: 'stop failed', flip: 'deploy failed', deploy: 'deploy failed' }[phase];
      detail = `unexpected during ${phase}: ${e.message}`;
      ctx.journal(`[${name}] attempt ${log.id} failed during ${phase}: ${e.stack ?? e}`);
    }
  }

  // Step 8: prune, after every outcome, unless a rollback is mid-acceptance for
  ...
  // Step 9: close. Wrapped: an I/O failure here (a full disk, a permissions
  // error) must not reject runEntry outright — that would skip step 10's
  ...
  // Step 10: notify. Cannot change the outcome; its own result is one events line.
...
async function failUnreadableState(ctx, entry, { dir, logDir, name, error }) {
  ...
  await appendEvent(logDir, 'started', `${log.id} ${entry.via ?? entry.kind} ${log.file}`);
  await log.line(`flipd attempt ${log.id}  name=${name}  trigger=${entry.via ?? entry.kind}`);
