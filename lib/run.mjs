...
export async function runEntry(ctx, entry) {
  ...
  const label = entry.via ?? entry.kind;
  const last = { attempt: log.id, trigger: label, sha: null, release: null, outcome: null, started: started.toISOString(), finished: null, log: log.file };
  ...
  await appendEvent(logDir, 'started', `${log.id} ${label} ${log.file}`);
  await log.line(`flipd attempt ${log.id}  name=${name}  trigger=${label}${entry.target ? `  target release=${entry.target}` : ''}`);
