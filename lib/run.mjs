...
export async function runEntry(ctx, entry) {
  ...
  const stop = async (releaseId, rel, previousSha) => {
    ...
    // cwd: Task 3 replaces this line with the current-release resolution.
    const cwd = path.join(dir, 'releases', releaseId, rel.root);
    await step('stop');
    const ok = await runPhase('stop', repo.stop, cwd, { releaseId, sha: rel.sha, previousSha, attemptId: log.id });
    await done('stop');
