...
// A parse error on a malformed env file can quote the offending line verbatim
// (loadEnvFile's "expected KEY=value, got \"...\"" shape) — and since a file that
// failed to parse contributes nothing to the log's own secret mask, that quoted
// text would otherwise reach the attempt log unredacted. Strip anything quoted.
const redactParseError = (message) => message.replace(/"[^"]*"/g, '"[elided]"');
...
export async function runEntry(ctx, entry) {
  ...
  const runPhase = async (which, command, cwd, ids) => {
    ...
    if (envError) throw new Stop(which === 'build' ? 'build failed' : 'deploy failed', `env file: ${redactParseError(envError.message)}`);
...
// Never throws. Takes the shutdown signal and forwards it to the command:
// a service that is stopping must be able to bound how long it waits on
// this (its caller is expected to await it, bounded, during shutdown — see
// serve()'s close()), rather than let it become an untracked child that
// outlives the process. `ctx.signal?.aborted` still shortens the cap for a
// job that starts already-doomed (a shutdown that began before this job
// even got to run); once running, an abort arriving mid-flight now reaches
// the command the same way it reaches every other command this codebase
// runs (a plain SIGTERM, escalating to SIGKILL after `graceMs` if ignored),
// not a full 60-second window regardless of what is waiting on it.
export async function runOnFailure(ctx, repo, { attemptId, outcome, sha, releaseId, logFile, envFile, mask }) {
  ...
  try {
    ...
    const scrub = makeScrubber(mask ?? [...extra.values()]);
    let out = '';
    const r = await runCommand({ command: repo.onFailure, cwd: p.repoDir(repo.name), env, timeoutSec: ctx.signal?.aborted ? 8 : 60, onOutput: (c) => { if (out.length < 65536) out += c; }, signal: ctx.signal, graceMs: 2000 });
