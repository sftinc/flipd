...
import { gitEnv, cloneBare, remoteUrl, fetchBranch, changedFiles, worktreeAdd, worktreeRemove, worktreePrune, submoduleUpdate, shortSha, isBareRepo, redactUserinfo, GitError } from './git.mjs';
    ...
    if (entry.kind === 'rollback') {
      ...
      try {
        ...
        if (await fs.stat(path.join(relDir, '.gitmodules')).then(() => true, () => false)) {
          ...
          await submoduleUpdate(relDir, gitRun);
        }
      } catch (e) {
