...
function confLines({ repoUrl, branch, root, build, deploy, sharedKey, hookHost }) {
  return [
    ...
    '#STOP=sudo /usr/local/bin/<your-drain-script>',
...
export default async function (args, { paths: p, stdout, stderr, forgeOverride }) {
  ...
  // An unreadable or malformed flipd.conf is a different problem than one that
  // has not been created yet (ENOENT, the installer-not-run-yet case, which
  // prints placeholders). Folding every error into "no PUBLIC_HOST" would print
  // the SSH recipe for a conf that has a PUBLIC_HOST and failed to parse.
  ...
  // A conf that loaded without PUBLIC_HOST is a box with no HTTP door: the
  // SSH recipe is the one that applies. ENOENT keeps the webhook recipe with
  // its placeholder, as before -- the installer may yet be run with --host.
  const step2 = main && !main.publicHost
    ? sshRecipe({ name, ghRepo })
    : webhookRecipe({ host, ghRepo, mainConf: p.mainConf });
...
async function addViaForge({ p, stdout, stderr, forgeOverride, account, main, mainErr, parsedUrl, name, conf, branch, root, build, deploy, sharedKey }) {
  ...
  // that simply has not been created yet: it will not be fixed by running
  // install.sh, so it gets its own message naming the file and the parse
  // error. ENOENT is refused too, on this path: there is no flipd on the box
  // yet, and this path wants to do the whole job.
  if (mainErr && mainErr.code !== 'ENOENT') {
    stderr.write(`${p.mainConf}: ${mainErr.message}\n`);
    return 1;
  }
  if (mainErr) {
    stderr.write(`${p.mainConf} not found; run install.sh first (with --host for a webhook), then add\n`);
    return 1;
  }
  // No PUBLIC_HOST is not an error: it is a box with no HTTP door. The deploy
  // key is still needed (it is for fetching), the webhook is not, and the
  // SSH recipe is printed in its place.
  const publicHost = main.publicHost;
  const hookUrl = publicHost ? `https://${publicHost}/deploy` : null;
  ...
  // parseRepoUrl drops the port on purpose (two spellings of one repository
  // compare equal), but ssh does not: off port 22 it looks the key up under
  // `[host]:port`, which is what `ssh-keyscan -p` writes. Read the port off
  // the URL directly so the printed command records a key ssh will match.
  const sshPort = /^ssh:\/\/[^/]*:(\d+)\//i.exec(remote.sshUrl)?.[1] ?? null;
  ...
  if (hookUrl) {
    ...
    existing = hooks.find((h) => normalizeHookUrl(h.url) === normalizeHookUrl(hookUrl)) ?? null;
  }

  // From here on there is something to undo. dirExisted is captured before
  // ensureKey runs — not read from ensureKey's own return — so undo knows
  // what to remove even if ensureKey itself throws before returning (its
  // mkdir can succeed and its chownFlipd or ssh-keygen can still fail after).
  // One extra stat here is cheap next to the ssh-keygen exec and the several
  // HTTPS calls this path is about to make; ensureKey's own dirExisted is the
  // redundant one, and is not read.
  const dir = p.repoDir(name);
  const dirExisted = await fs.stat(dir).then(() => true, () => false);
  ...
  try {
    ...
    if (hookUrl && !existing) { hookId = (await forge.addHook(owner, repo, { url: hookUrl, secret: main.webhookSecret })).id; hookCreated = true; }
  ...
  const hookLine = !hookUrl
    ? `  webhook            none (PUBLIC_HOST not set; trigger over SSH)`
    : existing
  ...
  const sshHostWarning = sshHostDiffers
    ? `\n  ssh host           ${sshHost}${sshPort ? `:${sshPort}` : ''} is not ${account.host} (a forge's SSH_DOMAIN setting can differ from its API host); record its key before the first build:\n                       ssh-keyscan ${sshPort ? `-p ${sshPort} ` : ''}${sshHost} >> ${p.knownHosts}\n`
    : '';
  const sshStep = hookUrl ? '' : `\n${sshRecipe({ name, ghRepo: `${owner}/${repo}` })}\n`;
  stdout.write(`wrote ${conf}${build && deploy ? '' : '   (BUILD and DEPLOY are placeholders: edit them)'}
${sharedKey ? `using ${sharedKey}` : `generated ${key}`}
${at}:
${keyLine}
${hookLine}${sshHostWarning}${sshStep}
