...
async function addViaForge({ p, stdout, stderr, forgeOverride, account, main, parsedUrl, name, conf, branch, root, build, deploy, sharedKey }) {
  ...
  // 3. Local key. From here on there is something to undo.
  const { dir, dirExisted, key, generated, pub } = await ensureKey(p, name, sharedKey);
  let keyId = null;
