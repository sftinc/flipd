...
//
// The sed that reads the secret out of flipd.conf is the same rule
// `parseKV` applies and the same one `install.sh`'s `read_secret` applies:
// last assignment wins, whitespace around the key, the '=' and the value is
// separator rather than value. A strict `^WEBHOOK_SECRET=` finds nothing in a
// legal hand-written ` WEBHOOK_SECRET = abc `, and the failure is silent —
// `gh` answers 201 for a webhook created with an empty secret, and every push
// it then delivers fails its HMAC.
...
export function webhookRecipe({ host, ghRepo, mainConf }) {
  return `add the webhook (Settings > Webhooks):
   ...
   SECRET=$(sudo sed -n 's/^[[:space:]]*WEBHOOK_SECRET[[:space:]]*=[[:space:]]*//p' ${mainConf} | tail -n1 | sed 's/[[:space:]]*$//') node -e '
