...
export default async function (args, { paths, stdout, stderr, sendOverride }) {
  ...
  for (const a of rest) {
    if (a === '--wait') wait = true;
    else return usage();   // as `run` treats anything but --now: a usage error, not silently dropped — a misspelt flag would otherwise look like it worked
  }
  ...
  try {
    ...
    // sendCommand's own message for a connection that closed with no reply —
    // most often the service dying mid-build under --wait — is `bad reply: `
    // (it tried to JSON.parse an empty buffer): accurate about the mechanism,
    // meaningless to read. Say what actually happened instead.
    const detail = e.code ?? (e.message === 'bad reply: ' ? 'connection closed before a reply arrived' : e.message);
    stderr.write(`service down (${detail}); is flipd.service running?\n`);
    return 3;
  }
